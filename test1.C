...
main(int argc,char **argv)
{
  ...
  for(i=0; i < Nrep;i++) {
    ...
    print_res(OUT,gdims2,grid2.ldims,grid2.glob_start);
    normalize(OUT,size2,gdims2);
    t -= MPI_Wtime();
    trans_b.exec(OUT,FIN,0);
    t += MPI_Wtime();
  }

  double mydiff = check_res(IN,FIN,grid1.ldims);
  double diff = 0.0;
...
{
  long int i;
  double f = 1.0/(gdims[0]*gdims[1]*gdims[2]);
...
void init_wave(double *IN,int *gdims,int *ldims,int *gstart)
{
  ...
  sinz = new double[gdims[2]];
   ...
   for(z=0;z < ldims[2];z++)
     sinz[z] = sin((z+gstart[2]-1)*twopi/gdims[2]);
   for(y=0;y < ldims[1];y++)
     siny[y] = sin((y+gstart[1]-1)*twopi/gdims[1]);
   for(x=0;x < ldims[0];x++)
     sinx[x] = sin((x+gstart[0]-1)*twopi/gdims[0]);
...
void print_res(complex_double *A,int *gdims,int *ldims,int *gstart)
{
  ...
  for(z=0;z < ldims[2];z++)
	...
	if(abs(*p) > Nglob *1.25e-4) 
	  cout << x+gstart[0] << y+gstart[1] << z+gstart[2] <<*p << endl;
	p++;
      }
}
