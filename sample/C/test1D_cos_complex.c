...
Here 
  ...
  dim is the dimension of 1D transform (valid values are 0 for X, through 2 for Z, and the logical dimension is specified, i.e. actual storage dimension may be different as specified by mem-order mapping).
...
main(int argc,char **argv)
{
  ...
  // Read input parameters
   ...
   if(myid == 0) {
     ...
     printf("P3DFFT test, 1D wave input, 1D cosine transform, complex\n");
#ifndef SINGLE_PREC
  ...
  type_ids1 = type_ids2 = P3DFFT_DCT1_COMPLEX_D;
  ...
  // Check that we have recovered the input
...
// Initialize a 3D complex array with 1D cosine wave (real part) in a given dimension
void init_wave1D(double *IN,int *mydims,int *gstart, int ar_dim)
{
  double *cos_coords,*p;
  int x,y,z;
  ...
  for(x=0;x < mydims[ar_dim];x++)
   ...
   case 0:
	 ...
	 for(x=0;x < mydims[0];x++) {
	   *p++ = cos_coords[x];
	   *p++ = 0.0;   /* Complex numbers with zero imaginary parts */
	 }
