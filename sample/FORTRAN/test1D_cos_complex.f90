!
!This program exemplifies the use of 1D transforms in P3DFFT++, for a 1D cosine transform, for complex-valued 
!arrays. 1D transforms are performed on 3D arrays, in the dimension specified as an argument. This could be an 
!isolated 1D transform or a stage in a multidimensional transform. This function can do local transposition, i.e. 
!arbitrary input and output memory ordering. However it does not do an inter-processor transpose (see test_transMPI 
!for that). 
!
...
! If you have questions please contact Dmitry Pekurovsky, dmitry@sdsc.edu
      ...
      complex(8), dimension(:,:,:),  allocatable :: BEG,C,AEND
...
! Set up 2 transform types for 1D transforms
      ...
      type_ids1 = P3DFFT_DCT1_COMPLEX_D

      type_ids2 = P3DFFT_DCT1_COMPLEX_D
...
!=========================================================
        ...
        integer mydims(3)
	complex(8) A(mydims(1),mydims(2),mydims(3))
...
! Initialize with 3D sine wave
      ...
      if(ar_dim .eq. 1) then
         ...
         do z=1,mydims(3)
            do y=1,mydims(2)
               do x=1,mydims(1)
                  A(x,y,z)=cmplx(cos_coords(x),0.0d0,8)
               enddo
            enddo
         enddo
         ...
         do z=1,mydims(3)
            do y=1,mydims(2)
               do x=1,mydims(1)
                  A(x,y,z)=cmplx(cos_coords(y),0.0d0,8)
               enddo
            enddo
         enddo
         ...
         do z=1,mydims(3)
            do y=1,mydims(2)
               do x=1,mydims(1)
                  A(x,y,z)=cmplx(cos_coords(z),0.0d0,8)
               enddo
            enddo
         enddo
...
!
      ...
      integer(8) i,Nar
      complex(8) Ar(mydims(1),mydims(2),mydims(3))
      real(8) Nglob
