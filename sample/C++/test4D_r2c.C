...
main(int argc,char **argv)
{
  ...
  MPI_Comm_rank(MPI_COMM_WORLD,&myid);
   ...
   if(myid == 0) {
     ...
     if((fp=fopen("stdin", "r"))==NULL){
        printf("Cannot open file. Setting to default N=128, ndim=2, Nrep=1.\n");
        for(i=0;i<4;i++) gdims[i]=128; 
	Nrep=1;ndim=2;
     } else {
       fscanf(fp,"%d %d %d %d %d %d\n",&N[0],&N[1],&N[2],&N[3],&ndim,&Nrep);
        fclose(fp);
     }
   ...
   MPI_Bcast(&N,4,MPI_INT,0,MPI_COMM_WORLD);
  ...
  init_wave(IN,sdims1,glob_start1,N);
