...
!
! This program initializes a 3D array with a 1D sine wave in chosen direction, 
then performs forward sine transform DST-1. Then in computes the inverse transform (the same in this case, and checks that
...
void init_wave1D(double *IN,int *gdims,int *ldims,int *gstart, int dim, int mem_order[3])
{
  ...
  for(x=0;x < gdims[dim];x++)
    mysin[x] = sin((x+1)*twopi/(2*(gdims[dim]+1)));
