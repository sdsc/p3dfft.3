...
void init_wave(float *,int[3],int *,int[3],int);
void print_res(complex<float> *,int *,int *,int *,int);
void normalize(complex<float> *,size_t,int *,int);
float check_res(float*,float *,int *,int);
void  check_res_forward(complex<float> *OUT,int sdims[3],int dimx,int glob_start[3], int gdims[3],int myid,int nv);
...
int main(int argc,char **argv)
{
  ...
  float *IN=new float[size1*nv];
  float *FIN=new float[size1*nv];
  ...
  complex<float> *OUT=new complex<float>[size2*nv];
  ...
  float mydiff = check_res(IN,FIN,sdims1,nv);
  float diff = 0.0;
...
void  check_res_forward(complex<float> *OUT,int sdims[3],int dimx,int glob_start[3], int gdims[3],int myid,int nv)
{
  ...
  for(z=0;z < sdims[2];z++) {
    ...
    else if(dimx != 2 && z + glob_start[2] == gdims[2]-iv-1)
      ans1 = complex<float>(0.0,-0.125);
    else
      ans1 = 0.0;
    for(y=0;y < sdims[1];y++) {
	...
	ans2 = ans1;
      else if(dimx != 1 && y + glob_start[1] == gdims[1]-iv-1)
	ans2 = -ans1;
      else
	ans2 = 0.0;
	...
	if(x + glob_start[0] == iv+1) 
	  ans = ans2;
	else if(dimx != 0 && x + glob_start[0] == gdims[0]-iv-1)
	  ans = -ans2;
	else
	  ans = 0.0;
...
void normalize(complex<float> *A,size_t size,int *gdims,int nv)
{
  size_t i;
...
void init_wave(float *IN,int *gdims,int *sdims,int *gstart,int nv)
{
...
void print_res(complex<float> *A,int *gdims,int *sdims,int *gstart,int nv)
{
  int x,y,z;
  complex<float> *p;
  float Nglob;
...
float check_res(float *A,float *B,int *sdims,int nv)
{
  int x,y,z;
  float *p1,*p2,mydiff;
  p1 = A;
